    ...
    <div class="l_moban"><!-- 模板 style -->
      ...
      <!-- 等待保证金返还的商品列表 -->
        <div class="h_content_mian l_all" style="margin-top:20px;">
            ...
            <div class="h_mian_bt  l_all">已完成商品表</div>
                ...
                <div class=" h_content_all l_all dongtai pagen" value="${pagenow}">
                       <c:forEach items="${finushlist}" var="finushlist">    
                        <ul class="h_contents l_all ">
                            ...
                            <li class="w20">${finushlist.goods_num }</li>                            
                            <li class="w20">${finushlist.goods_name}</li> 
                            <li class="w15">${finushlist.goods_rmb }</li>                                                 
                            <li class="w25 ">
                                <a href="<%=basePath%>/storeSingleBusiness.do?publish_ready_detailsend&id=${finushlist.id}" class="h_mx dk fl bianji tan4" style="color:#fff !important;width:74px;">查看详情</a>
...
/* 调用弹框*/
 ...
 $(".searchwhy").bind('click',function(){
		...
		$.post(
				url='<%=basePath%>/storeSingleBusiness.do?publish_goods_finish',
				data={
						search : search,
						pagenow : 1
						},
