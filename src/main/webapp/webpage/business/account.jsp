 ...
 <div class="h_content_mian l_all">
            ...
            <div class=" h_content_all l_all dongtai pagen"  value="${pagenow }">
                    ...
                    <ul class="h_contents l_all ">
                        <li class="w25">${list.create_time }</li>
                        <li class="w25">${list.serial_num}期商品${list.goods_name }交付保证金+服务费</li>
                        <li class="w25 income"> 
                        <c:choose>
					    ...
					    <c:choose>
							<c:when test="${list.payment_status=='1'}">  
    		  					<li class="w25">${list.deposit+list.servicefee }</li> 
							</c:when>
							<c:otherwise> 
			 					<li class="w25">${list.deposit}</li> 
							</c:otherwise>
					    </c:choose>
		...
		<script>
		    $(".tcdPageCode").createPage({
		        ...
		        backFn:function(p){
		    		...
		    		if(name==null){name="";}
		    		$.post(
		    				url='<%=basePath%>/storeBusiness.do?account',
		    				data={
		    						pagenow : p,
		    						type : type,
		    						order : why,
