...
public static JSONObject sendGetRequest(String reqURL){
	//实现json返回
	JSONObject jsonObject = null;
	
	String respContent = "{\"msg\":\"通信失败\"}"; //响应内容 
	...
	try{
		...
		HttpEntity entity = response.getEntity();			//获取响应实体 
            ...
            LogUtil.error("请求通信[" + reqURL + "]时连接超时,堆栈轨迹如下", cte);
        } catch (SocketTimeoutException ste){
            LogUtil.error("请求通信[" + reqURL + "]时读取超时,堆栈轨迹如下", ste);
        }catch(ClientProtocolException cpe){
            //该异常通常是协议错误导致:比如构造HttpGet对象时传入协议不对(将'http'写成'htp')or响应内容不符合HTTP协议要求等
            LogUtil.error("请求通信[" + reqURL + "]时协议异常,堆栈轨迹如下", cpe);
        }catch(ParseException pe){
            LogUtil.error("请求通信[" + reqURL + "]时解析异常,堆栈轨迹如下", pe);
        }catch(IOException ioe){
            //该异常通常是网络原因引起的,如HTTP服务器未启动等
            LogUtil.error("请求通信[" + reqURL + "]时网络异常,堆栈轨迹如下", ioe);
        }catch (Exception e){
            LogUtil.error("请求通信[" + reqURL + "]时偶遇异常,堆栈轨迹如下", e); 
        }finally{ 
            //关闭连接,释放资源 
            httpClient.getConnectionManager().shutdown();
        }
        jsonObject = JSONObject.fromObject(respContent);
        return jsonObject; 
    }
     ...
     * @return 远程主机响应正文
     */ 
    public static JSONObject sendPostRequest(String reqURL, String reqData, String encodeCharset){ 
    	//实现json返回
    	JSONObject jsonObject = null;
    	
    	String reseContent = "{\"msg\":\"通信失败\"}"; //响应内容 
        ...
        jsonObject = JSONObject.fromObject(reseContent);
        return jsonObject; 
    } 
