...
@Scope("prototype")
@Controller
@RequestMapping(value="/app_lydb/alipayZeroBusiness")
public class AlipayZero extends BaseController{
	private static final Logger logger = Logger.getLogger(AlipayZero.class);
	@Autowired
	private DbZeroGoodsServiceI dbZeroGoodsService;
	@Autowired
	private DbZgoodsPaymentServiceI dbZgoodsPaymentService;
	@Autowired
...
//    	PrintWriter out = response.getWriter();
        ...
        DbZeroGoodsEntity en = dbZeroGoodsService.getEntity(DbZeroGoodsEntity.class, id);
        storeController store=new storeController();

		payment.setStarttime(store.Time());
        payment.setOrderNum(UUIDGenerator.generate());
		dbZgoodsPaymentService.saveOrUpdate(payment);
        // 支付类型  
        // 必填，不能修改  
        String payment_type = "1";  
        // 服务器异步通知页面路径  
        // 需http://格式的完整路径，不能加?id=123这类自定义参数  
        String notify_url = "http://dbback.stzero.cn/xingluo/rest/app_lydb/alipayZeroBusiness/async";
        // 页面跳转同步通知页面路径  
        // 需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/  
        String return_url = "http://dbback.stzero.cn/xingluo/rest/app_lydb/alipayZeroBusiness/returnurl";
        // 商户订单号.  
        // 商户网站订单系统中唯一订单号，必填  
        //String out_trade_no = date.getTime() + "";  
        // 订单名称  
        // 必填  
        String subject = en.getZgoodsName();
        // 防钓鱼时间戳  
		...
		sParaTemp.put("out_trade_no",payment.getOrderNum());//商品订单编号
	...
	public String async(HttpServletRequest request,
		 ...
		 Map<String,String> params = new HashMap<String,String>();  
	        ...
	        if(AlipayNotify.verify(params)){//验证成功  
	            if(tradeStatus.equals("TRADE_FINISHED") || tradeStatus.equals("TRADE_SUCCESS")) {  
                    System.out.println(">>>>>充值成功" + tradeNo);
					DbZgoodsPaymentEntity pay=systemService.findUniqueByProperty(DbZgoodsPaymentEntity.class,"orderNum" ,tradeNo);
					...
					try {
						...
						dbZgoodsPaymentService.saveOrUpdate(pay);
						goods.setStatus("4");
						dbZeroGoodsService.saveOrUpdate(goods);
	           ...
	           /* return "web/pay/success";*/  
	            ...
	            /*return "web/pay/fail"; */ 
	        logger.info(DateUtils.getDate().toString()+"支付失败");
	        return "fail";
	        }  
